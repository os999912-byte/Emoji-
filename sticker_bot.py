...
# ── Получение файла для InputSticker ─────────────────────────────────────────

async def get_sticker_data(bot, msg):
    """
    Возвращает (data, fmt):
      data — bytes для любых загружаемых файлов,
             str (file_id) только для готовых TG-стикеров.

    ВАЖНО: InputSticker принимает str | bytes | BytesIO.
    InputFile передавать НЕЛЬЗЯ — будет ошибка 'field sticker must be of type string'.
    """
    ...
    if msg.sticker:
        ...
        return msg.sticker.file_id, fmt   # file_id готового стикера — разрешён
    ...
    if msg.document:
        ...
        return await dl(d.file_id), fmt

    if msg.photo:
        return await dl(msg.photo[-1].file_id), StickerFormat.STATIC
...
async def receive_pack_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        "Только латиница, цифры, <code>_</code>, от 5 до 64 символов.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(kb),
    )
    return PACK_LINK
...
async def _prompt_first_sticker(update: Update, suffix: str) -> int:
    text = (
        ...
        "<i>Фото, PNG, WEBP, WEBM, TGS или готовый стикер TG.\n"
        "После файла я попрошу указать эмодзи.</i>"
    )
    if update.callback_query:
...
async def receive_pack_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not suffix.replace("_", "").isalnum() or not (5 <= len(suffix) <= 64):
        await update.message.reply_text(
...
async def receive_sticker_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    data, fmt = await get_sticker_data(context.bot, update.message)
    if data is None:
...
# ── Создание: шаг 5 — эмодзи + вызов API ─────────────────────────────────────
...
async def receive_sticker_emoji(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    input_sticker = InputSticker(
        sticker=data,           # bytes или str (file_id готового стикера)
