...
async def auto_detect_emoji(image_bytes: bytes) -> list[str]:
    """
    Отправляет изображение в Claude и получает список подходящих эмодзи.
    Возвращает список из 1-3 эмодзи. При ошибке — ["🙂"].
    """
    ...
    try:
            ...
            "messages": [{
                ...
                "content": [
                    ...
                    },
                    {
                        "type": "text",
                        "text": (
                            "Look at this sticker image and reply with 1-3 fitting emoji only. "
                            "No text, no punctuation, no spaces — just the emoji characters themselves. "
                            "Example: 😎🔥"
                        ),
                    },
                ],
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            # Берём только символы-эмодзи
            import unicodedata
            emojis = [ch for ch in text if unicodedata.category(ch) in ("So", "Sm", "Sk") or ord(ch) > 0x1F300]
            return emojis[:3] if emojis else ["🙂"]
    except Exception as e:
        logger.warning("auto_detect_emoji error: %s", e)
        return ["🙂"]


def is_auto_emoji(context) -> bool:
    """Проверяет включено ли автоопределение эмодзи для пользователя."""
    return context.user_data.get("auto_emoji", True)  # По умолчанию включено
...
async def _ask_emoji_for_next(update_or_msg, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if is_auto_emoji(context):
        ...
        for i in range(done, total):
            ...
            emoji_list = await auto_detect_emoji(data if isinstance(data, bytes) else b"")
