...
def process_image_for_sticker(raw_bytes: bytes, pack_type: str = "sticker") -> bytes:
    """
    Принимает любое изображение (JPEG, PNG, WEBP, BMP...) и возвращает
    корректный PNG:
      - стикер: ровно 512×512, прозрачный фон
      - эмодзи: ровно 100×100, прозрачный фон
    Файл сжимается до MAX_STICKER_BYTES.
    """
    size = 512 if pack_type == "sticker" else 100

    img = Image.open(io.BytesIO(raw_bytes))

    # Конвертируем в RGBA чтобы сохранить прозрачность
    if img.mode != "RGBA":
        img = img.convert("RGBA")

    # Вписываем изображение в квадрат с сохранением пропорций (thumbnail)
    img.thumbnail((size, size), Image.LANCZOS)

    # Создаём квадратный холст с прозрачным фоном
    canvas = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    offset = ((size - img.width) // 2, (size - img.height) // 2)
    canvas.paste(img, offset, img)

    # Сохраняем как PNG, уменьшая качество если нужно
    ...
    scale = 0.9
    while len(data) > MAX_STICKER_BYTES and scale > 0.3:
        new_size = (int(size * scale), int(size * scale))
        resized = canvas.resize(new_size, Image.LANCZOS)
        final   = Image.new("RGBA", (size, size), (0, 0, 0, 0))
        off     = ((size - new_size[0]) // 2, (size - new_size[1]) // 2)
...
async def send_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    text = (
        ...
        "<i>Просто отправь любое фото или файл — я сам всё подготовлю!</i>"
    )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        f"Привет, {name}!\n\nЯ создаю <b>настоящие</b> стикер-паки и premium emoji.\n"
        f"Просто отправь любое фото — я сам всё подготовлю 🚀",
        parse_mode="HTML",
...
async def receive_pack_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        "Я автоматически исправлю недопустимые символы.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(kb),
    )
    return PACK_LINK
...
async def _prompt_first_sticker(update: Update, suffix: str) -> int:
    text = (
        ...
        "<i>Любое фото, PNG, WEBP, JPEG — я сам подготовлю файл для Telegram.</i>"
    )
    if update.callback_query:
...
async def add_more(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("Отправь следующий файл — любое фото или изображение 👇")
    return ADDING_STICKER
...
async def mgmt_add(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        ...
        f"<i>Любое фото, PNG, WEBP — я подготовлю сам.</i>",
        parse_mode="HTML",
