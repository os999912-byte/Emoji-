...
# ── Авто-определение эмодзи по цвету (локально, без API) ─────────────────────

def auto_detect_emoji(image_bytes: bytes) -> list:
    """
    Анализирует изображение локально по цвету, яркости и насыщенности.
    Без внешних API. Возвращает 1-2 подходящих эмодзи.
    """
    try:
        ...
        transparency_ratio = 1 - total / len(pixels)

        red_px    = sum(1 for r,g,b,_ in opaque if r>150 and r>g*1.3 and r>b*1.3)
        green_px  = sum(1 for r,g,b,_ in opaque if g>150 and g>r*1.2 and g>b*1.2)
        blue_px   = sum(1 for r,g,b,_ in opaque if b>150 and b>r*1.2 and b>g*1.2)
        yellow_px = sum(1 for r,g,b,_ in opaque if r>180 and g>150 and b<100)
        orange_px = sum(1 for r,g,b,_ in opaque if r>200 and 80<g<180 and b<80)
        pink_px   = sum(1 for r,g,b,_ in opaque if r>200 and g<160 and b>140)
        purple_px = sum(1 for r,g,b,_ in opaque if r>100 and b>150 and b>g*1.2 and r<200)
        brown_px  = sum(1 for r,g,b,_ in opaque if r>100 and g>50 and b<80 and r>g*1.3)
        white_px  = sum(1 for r,g,b,_ in opaque if r>220 and g>220 and b>220)
        black_px  = sum(1 for r,g,b,_ in opaque if r<50 and g<50 and b<50)
        cyan_px   = sum(1 for r,g,b,_ in opaque if b>150 and g>150 and r<120)

        def pct(n): return n / total
        ...
        result = []
        for count, threshold, main_emoji, extra_emoji in COLOR_MAP:
            if pct(count) >= threshold:
                result.append(main_emoji)
                if saturation > 100 and brightness > 150:
                    result.append(extra_emoji)
                elif transparency_ratio > 0.55:
                    result.append("💫")
                elif brightness < 70:
                    result.append("🌑")
                break
...
async def _ask_emoji_for_next(update_or_msg, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for i in range(done, total):
        ...
        if err:
            ...
            await msg.reply_text(f"⚠️ Файл #{i+1} пропущен:\n{err}")
        else:
